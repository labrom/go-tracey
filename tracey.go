...
func New(opts *Options) (func(fmt.Stringer), func(...interface{}) fmt.Stringer) {
	var options Options
	if opts != nil {
		options = *opts
	}
	...
	if options.DisableTracing {
		return func(fmt.Stringer) {}, func(...interface{}) fmt.Stringer { return stringer("") }
	}
	...
	_enter := func(args ...interface{}) fmt.Stringer {
		...
		if options.EnableTiming {
			return Meta{traceMessage, time.Now()}
		} else {
			return stringer(traceMessage)
		}
	}

	// Exit function, invoked on function exit (usually deferred)
	_exit := func(s fmt.Stringer) {
		_decrementDepth()
		if reflect.TypeOf(s) == reflect.TypeOf(Meta{}) {
			options.CustomLogger.Printf("%s%s%s - spent %v\n", _spacify(), options.ExitMessage, s.(Meta).format, time.Now().Sub(s.(Meta).start))
		} else {
